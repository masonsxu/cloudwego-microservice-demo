...
<p align="center">
  <img src="images/architecture.svg" alt="系统架构图" width="100%">
</p>
...
flowchart LR
    ...
    subgraph Observability["可观测性"]
        MW1["OTel Tracing<br/><small>分布式追踪</small>"]
        MW2["RequestID<br/><small>请求标识生成</small>"]
    end

    subgraph Common["通用处理"]
        MW3["ResponseHeader<br/><small>标准响应头</small>"]
        MW4["Trace<br/><small>追踪信息注入 metainfo</small>"]
        MW5["CORS<br/><small>跨域资源共享</small>"]
        MW6["ErrorHandler<br/><small>统一错误捕获</small>"]
    end

    subgraph Security["安全控制"]
        MW7["JWT<br/><small>身份认证</small>"]
        MW8["Casbin<br/><small>RBAC 权限检查</small>"]
    end

    subgraph Cache["缓存"]
        MW9["ETag<br/><small>缓存协商</small>"]
    end

    MW1 --> MW2 --> MW3 --> MW4 --> MW5 --> MW6 --> MW7 --> MW8 --> MW9

    MW9 --> Handler([业务 Handler])
```

> 来源：`gateway/internal/wire/server.go:72-84`
...
<p align="center">
  <img src="images/service-interaction.svg" alt="服务交互流程图" width="100%">
</p>
...
<p align="center">
  <img src="images/data-model.svg" alt="数据模型关系图" width="100%">
</p>
...
**决策**：采用三层权限模型，将认证、粗粒度授权、细粒度授权分离到不同位置。

| 层 | 职责 | 位置 |
|---|---|---|
| 认证 AuthN | 验证身份（仅验签 + 注入 header） | 网关 `jwt_middleware` |
| 粗粒度授权 Coarse AuthZ | 路由级 ACL（路径前缀 + 角色 OR） | 网关 `authz_middleware` |
| 细粒度授权 Fine AuthZ | 数据级 ABAC/RBAC | 独立 PDP 服务 `policy_srv` |

> **权限模型已完成重构**：Phase 0–5 已全部落地（gateway 瘦身、JWT 最小化 +
> RS256 + JWKS、独立 PDP 服务、iamclient SDK、identity_srv 接入）。
> 设计与接入指南详见 [`../04-权限管理/权限管理设计.md`](../04-权限管理/权限管理设计.md)
> 与 [`../04-权限管理/SDK使用指南.md`](../04-权限管理/SDK使用指南.md)。

**优势**：
- 网关无状态：不持有任何业务库连接，可水平扩展
- JWT 体积稳定：仅承载稳定身份，被禁用账号实时失效由 PDP 兜底
- 业务系统独立鉴权：通过 SDK 调 PDP，不依赖网关代理
- 单点策略管理：策略变更通知机制集中在 `policy_srv`

...
### 开发环境（podman kube play 一键全栈）

```bash
# 1. 准备配置（首次使用）
cp docker/pod.yml.example docker/pod.yml
vim docker/pod.yml   # 修改 Secret 中四个 CHANGE_ME_* 占位密码

# 2. 构建 Go 服务镜像
./scripts/build-images.sh

# 3. 一键启动全栈（基础设施 + identity_srv + gateway，共 7 个 Pod）
...
# 查看日志
podman logs -f gateway-pod-gateway
podman logs -f identity-srv-pod-identity-srv
```

### 生产环境

生产环境直接将 `docker/pod.yml` 转换为标准 K8s 资源应用到目标集群（podman kube generate / kubectl apply）。`pod.yml` 已使用 K8s 原生资源类型（Secret/ConfigMap/PVC/Deployment/Service），与真实 K8s 集群兼容。
...
```bash
podman kube play docker/pod.yml                    # 启动基础设施
podman kube play --down docker/pod.yml             # 停止基础设施
podman pod ps                                      # 查看服务状态
podman kube play --down --force docker/pod.yml     # 清理环境（删除数据）
```
...
```bash
podman logs -f            # 所有日志
podman logs -f postgres-pod-postgres   # 特定服务日志
podman logs -f redis-pod-redis      # Redis 日志
```

...
# Docker/Podman 环境清理
podman kube play --down docker/pod.yml && podman kube play docker/pod.yml
```
...
1. 启动数据库：`podman kube play docker/pod.yml`
...
1. 启动 etcd：`podman kube play docker/pod.yml`
...
1. 启动 Redis：`podman kube play docker/pod.yml`
...
# 基础设施日志
podman logs -f
podman logs -f postgres-pod-postgres    # PostgreSQL 日志

...
> ⚠️ **DEPRECATED — 本文档已废弃**
>
> 本文档描述的权限模型（网关内嵌 Casbin、JWT 携带 `RoleIDs/DepartmentIDs/CorePermission/DataScope` 等业务字段、网关从 identity_srv 同步策略）**已被否定**。新模型边界、设计原则与目标态实现请参考：
>
> - **重构执行参考**：[`重构提案-网关边界与权限模型.md`](重构提案-网关边界与权限模型.md)（含 Phase 0–5 执行计划与 Q1–Q7 决策记录）
> - **目标态设计**：[`权限管理设计-v2.md`](权限管理设计-v2.md)
>
> 重构进入 Phase 5 后本文档将被删除，v2 文档重命名为本文件。在此之前本文档仍可作为旧实现的事实记录，但**不要据此设计或实现新代码**。
...
- 白名单接口正常，白名单外接口持续 403
- 日志反复出现 `Casbin denied: no matching policy found`
- superadmin 也被拒绝

#### 根因拆解（四层）

1. **同步闭环缺失**：gateway 侧 `SyncPolicies` 仅清空本地 enforcer，未加载 `p/g/g2` 策略明细。
2. **资源标识不一致**：请求资源使用原始 path（如 `/api/v1/permission/roles`），而策略对象是 `menu:*`。
3. **动作匹配缺失**：过滤策略时仅匹配精确 `act`，未兼容 `p.act = "*"`。
4. **主体维度不一致**：JWT 中多为 `role:<uuid>`，策略主体可能是 `role:<code>`（如 `role:superadmin`）。

#### 修复清单

1. 扩展 `SyncPolicies` 响应，回传可装载策略明细：`policies / groupingPolicies / roleInheritancePolicies`。
2. gateway 同步后执行完整重建：
   - `ClearPolicy()`
   - `AddPolicies(p)`
   - `AddGroupingPolicies(g)`
   - `AddNamedGroupingPolicies("g2", g2)`
3. 将 `PolicySyncService` 接入 wire 生命周期：启动时 `Start`，进程退出时 `Stop`。
4. 补齐 `path -> menu:<id>` 映射（至少覆盖 permission 与 identity 的受保护路由）。
5. `EnforceWithDataScope` 合并匹配 `act` 与 `*` 两类策略。
6. 多角色检查时，主体集合同时包含：
   - JWT roleID 直接映射的 `role:<uuid>`
   - 通过 `g` 关系解析出的 `role:<code>`

#### 验证步骤

1. 启动后确认同步日志中 `p/g/g2` 加载数量均符合预期（非异常空集）。
2. 复测受保护接口：
   - superadmin：应通过（非 403）
   - 无权限角色：应继续拒绝（403）
3. 重启 identity_srv + gateway 后复测，确认策略数量稳定、无重复膨胀。

#### 排障检查顺序（建议固定）

1. **先看同步**：是否回传并加载了 `p/g/g2`。
2. **再看资源**：日志中的 `resource` 是否已映射为 `menu:*`。
3. **再看动作**：是否命中 `act=*` 通配策略。
4. **最后看主体**：请求主体是否同时覆盖 `role:<uuid>` 与 `role:<code>`。
